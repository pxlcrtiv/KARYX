...
def create_air_gap_package(optimized_model_path: str, audit_log: Dict[str, Any], target_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    The package embeds a COMPLIANCE_REPORT.json and a license-status suffix
    in the tarball filename so reviewers can see accreditation status at a
    glance. The watermark itself lives inside the audit log (written by
    AuditLogger) and is carried here verbatim.
    """
    ...
    # License watermark (from the audit log header) drives the package status.
    watermark = audit_log.get("header", {}).get("license_watermark", {})
    if watermark.get("license_status") == "VALID":
        license_suffix = "LICENSED"
    elif watermark.get("license_status") == "EVALUATION":
        license_suffix = "EVALUATION"
    else:
        license_suffix = "UNLICENSED"
    ...
    # 2b. Compliance report — what reviewers open first.
    compliance_report = {
        "package_id": package_id,
        "compliance_status": (
            "ACCREDITED" if watermark.get("accreditation_ready") else "NOT_ACCREDITED"
        ),
        "license_status": watermark.get("license_status", "UNKNOWN"),
        "license_type": watermark.get("license_type", "none"),
        "accreditation_ready": bool(watermark.get("accreditation_ready", False)),
        "auditor_notes": watermark.get("auditor_notes", ""),
        "generated_at": audit_log.get("header", {}).get("end_time"),
        "warning": None if watermark.get("accreditation_ready") else (
            "THIS PACKAGE IS NOT ACCREDITED FOR GOVERNMENT PRODUCTION USE. "
            "It was produced without a valid Karyx commercial license."
        ),
    }
    with open(os.path.join(temp_dir, "COMPLIANCE_REPORT.json"), "w") as f:
    ...
    # 4. Packaging (filename carries the classification level AND license status)
    tar_path = f"karyx_{package_id}.{audit_log['header']['classification']}.{license_suffix}.tar.gz"
